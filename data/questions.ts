...
export const sampleQuizzes: Quiz[] = [
  ...
  // NOUVEAUX QUIZ AVEC VARIANTES
  {
    id: 'quiz-6',
    title: 'Vrai ou Faux - Les Dogmes',
    description: 'Testez vos connaissances avec des questions vrai/faux sur les dogmes de la foi',
    category: 'dogmes',
    level: 1,
    questions: sampleQuestions.filter(q => q.questionType === 'true-false' && q.category === 'dogmes'),
    passingScore: 75,
    timeLimit: 10
  },
  {
    id: 'quiz-7',
    title: 'Reconnaissance d\'Images - Saints et Objets Liturgiques',
    description: 'Identifiez les saints et objets liturgiques à partir d\'images',
    category: 'saints',
    level: 1,
    questions: sampleQuestions.filter(q => q.questionType === 'image-recognition'),
    passingScore: 80,
    timeLimit: 12
  },
  {
    id: 'quiz-8',
    title: 'Citations Bibliques à Compléter',
    description: 'Complétez les citations célèbres de la Bible',
    category: 'saintes-ecritures',
    level: 1,
    questions: sampleQuestions.filter(q => q.questionType === 'quote-completion'),
    passingScore: 70,
    timeLimit: 15
  },
  {
    id: 'quiz-9',
    title: 'Mélange de Variantes - Fondamentaux',
    description: 'Un quiz varié avec différents types de questions sur les fondamentaux',
    category: 'dogmes',
    level: 1,
    questions: [
      ...sampleQuestions.filter(q => q.category === 'dogmes' && q.questionType === 'multiple-choice').slice(0, 5),
      ...sampleQuestions.filter(q => q.questionType === 'true-false' && q.category === 'dogmes'),
      ...sampleQuestions.filter(q => q.questionType === 'quote-completion' && q.category === 'saintes-ecritures').slice(0, 2)
    ],
    passingScore: 75,
    timeLimit: 20
  },
  {
    id: 'quiz-10',
    title: 'Quiz Avancé - Toutes Variantes',
    description: 'Un quiz complet avec tous les types de questions pour les utilisateurs expérimentés',
    category: 'dogmes',
    level: 2,
    questions: [
      ...sampleQuestions.filter(q => q.difficulty === 'difficile').slice(0, 3),
      ...sampleQuestions.filter(q => q.questionType === 'true-false').slice(0, 2),
      ...sampleQuestions.filter(q => q.questionType === 'image-recognition').slice(0, 2),
      ...sampleQuestions.filter(q => q.questionType === 'quote-completion').slice(0, 3)
    ],
    passingScore: 80,
    timeLimit: 25
  },
...
export function getQuizzesByLevel(level: number, userLevel: number, isAdmin: boolean = false): Quiz[] {
  const availableQuizzes = getAvailableQuizzes(userLevel, isAdmin);
